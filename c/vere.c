...
u2_ve_getopt(c3_i argc, c3_c** argv)
{
  ...
  while ( (ch_i = getopt(argc, argv, "s:b:v")) != -1 ) {
    switch ( ch_i ) {
      case 's': {
        c3_w kgo_w = atoi(optarg);

        opt = u2_ckd_by_put(opt, c3__kgo, kgo_w);
        break;
      }
      case 'b': {
...
{
  fprintf(stderr, "%s: usage: [-v -s] [-b kernel] computer\n", argv[0]);
  exit(1);
}
...
u2_ve_build(u2_noun tah)
{
  ...
  if ( !src_d ) {
    ...
    {
      fprintf(stderr, "%d: no %d kernel", kno_w, kno_w + 1);
...
/* u2_ve_stage(): load current stage.  yes iff stage is loaded at all.
*/
u2_flag
u2_ve_stage()
{
  ...
  sprintf(kna_c, "%d", kno_w);
  kna = u2_ci_string(kna_c);

  bot = u2nc(c3__boot, u2_ci_string(kna_c));
     ...
     c3_c** argv)
{
  ...
  if ( 0 != u2_cm_trap() ) {
    ...
    //  Set boot and goal stages.
    {
      ...
      u2_noun kgo = u2_ckd_by_get(u2_ct(u2_Host.opt), c3__kgo);
      ...
      if ( u2_none == kgo ) { 
        ...
        kto_w = (kgo > kno_w) ? 0 : (kno_w - kgo);
      }

      if ( kno_w == kto_w ) {
        fprintf(stderr, "%s: %s: init %d\n", 
                argv[0], u2_Local, kno_w); 
      } else {
        fprintf(stderr, "%s: %s: init %d, to %d\n", 
                argv[0], u2_Local, kno_w, kto_w); 
      }
    }
    ...
    {
      u2_noun kat_w = 0;
      // u2_noun bug, sab;      // debug & profile dumps
      ...
      while ( 1 ) {
        ...
        if ( 0 != u2_cm_trap() ) {
          ...
          fprintf(stderr, "main failed - debug\n");
          fin = u2_no;
        } else {
          u2_Host.kno_w = kno_w;
      ...
      if ( 0 == kat_w ) {
        fprintf(stderr, "%s: %s: boot %d: no boot\n", argv[0], u2_Local, kno_w);
        exit(1);
      } else {
        fprintf(stderr, "%s: %s: boot %d\n", argv[0], u2_Local, kat_w);
