...
*/
                  ...
                  u4_tack tac)
  {
    ...
    if ( u4_n_zero(tac) ) {
        ...
        if ( u4_n_eq(p_sut, p_tac) ) {
          return u4_kt
            (lan, 
             u4_atom_face, 
             p_tac, 
             _iris_burn_dext(p, q_sut, bar, axe, vef, q_tac));
        }
        else {
          return u4_kt
            (lan,
             u4_atom_face,
             p_tac,
             _iris_burn_dext(p, sut, bar, axe, vef, q_tac));
        }
      }
      ...
      {
        return _iris_burn_sint(p, sut, bar, axe, vef, tac);
      }
      else {
        return u4_kt
          (lan,
      ...
      if ( u4_b_pq(sut, u4_atom_cell, &p_sut, &q_sut) ) {
        ...
        _iris_slip(p, &l_sut, &l_bar, &l_axe, u4_noun_2);

        r_sut = sut; r_bar = bar; r_axe = axe;
        _iris_slip(p, &r_sut, &r_bar, &r_axe, u4_noun_3);
        ...
        u4_noun l_sut, l_bar, l_axe;
        u4_noun pq_sut, qq_sut;

        l_sut = sut; l_bar = bar; l_axe = axe;
        _iris_slip(p, &l_sut, &l_bar, &l_axe, u4_noun_2);

        if ( u4_b_p(q_sut, u4_atom_hard, &pq_sut) ) {
          u4_type ham = _iris_burn_dext(p, l_sut, l_bar, l_axe, u4_no, p_tac);
          ...
          u4_type ham = _iris_burn_dext(p, l_sut, l_bar, l_axe, vef, p_tac);
                  ...
                  u4_tack tac)
  {
    ...
    if ( u4_b_pq(sut, u4_atom_fork, &p_sut, &q_sut) ) {
      u4_unit yop = _iris_burn_swim(p, p_sut, bar, axe, vef, tac);
      u4_unit paf = _iris_burn_swim(p, q_sut, bar, axe, vef, tac);
      ...
      if ( u4_so(_iris_cull(p, sut, bar, axe)) ) {
        ...
        return u4_kc
          (lan, u4_noun_0, _iris_burn_dext(p, sut, bar, axe, vef, tac));
      }
    }
  }
  static u4_type
    ...
    if ( u4_b_pq(sut, u4_atom_face, &p_sut, &q_sut) ) {
      return u4_kt
        (lan, 
         u4_atom_face,
         p_sut,
         _iris_burn_dext(p, q_sut, bar, axe, vef, tac));
    }
    else if ( u4_b_pq(sut, u4_atom_fork, &p_sut, &q_sut) ) {
      return u4_ct(_iris_burn_swim(p, sut, bar, axe, vef, tac));
    }
      ...
      return _iris_burn_dext
        (p, _rose_repo(p, p_sut, q_sut), bar, axe, vef, tac); 
    }
    else return u4_trip;
  }
u4_type
...
*/
                  ...
                  u4_term cog)
  {
    ...
    if ( u4_b_pq(sut, u4_atom_fork, &p_sut, &q_sut) ) {
                ...
                u4_bag_in(u4_kc(lan, bar, sut), gil) )
      {
        ...
        return u4_kc
          (lan, u4_noun_0, _iris_find_main(p, sut, bar, axe, gil, cog));
      }
    }
  }
  static u4_unit
                    ...
                    u4_term cog)
  {
         ...
         u4_b_p(sut, u4_atom_cube, &p_sut) )
    {
      ...
      if ( !u4_b_p(q_sut, u4_atom_hard, &dab) ) {
        u4_b_pq(q_sut, u4_atom_soft, 0, &dab);
      }
      zem = _gull_look(p, dab, cog);
      if ( u4_n_zero(zem) ) {
        return _iris_find_half(p, sut, bar, axe, gil, cog, u4_noun_2);
      }
      else return 
        u4_kc
          (lan, 
      ...
      if ( u4_n_eq(cog, p_sut) ) {
        return u4_kq
          (lan, u4_noun_0, axe, u4_noun_0, q_sut);
      }
                  ...
                  u4_pool vit)
  {
    ...
    if ( u4_b_pq(sut, u4_atom_fork, &p_sut, &q_sut) ) {
      ...
      if ( u4_so(_iris_cull(p, sut, bar, axe)) ) {
        ...
        return u4_kc
          (lan, u4_noun_0, _iris_fish_main(p, sut, bar, axe, vit));
      }
    }
  }
  static u4_tool
    ...
    if ( u4_n_eq(u4_atom_atom, sut) ) {
      ...
      return _lily_flan
        (p,
         ...
         _lily_flan
          (p, _iris_fish_slip(p, sut, bar, axe, vit, u4_noun_2),
              _iris_fish_slip(p, sut, bar, axe, vit, u4_noun_3)));
    }
    else if ( u4_b_pq(sut, u4_atom_face, &p_sut, &q_sut) ) {
...
_iris_fish(u4_crow p,
           ...
           u4_axis axe)
{
  return _iris_fish_main(p, sut, bar, axe, u4_noun_0);
}
                  ...
                  u4_axis had)
  {
    ...
    if ( u4_b_pq(sut, u4_atom_fork, &p_sut, &q_sut) ) {
      ...
      if ( u4_n_zero(doz) ) return ryg;
        ...
        (lan, u4_noun_0, _rose_eith(p, u4_ct(doz), u4_ct(ryg)));
    }
    else {
      if ( u4_so(_iris_cull(p, sut, bar, axe)) ) {
        return u4_noun_0;
        ...
        return u4_kc
          (lan, u4_noun_0, _iris_half(p, sut, bar, axe, had));
      }
    }
  }
u4_type
      ...
      if ( u4_b_pq(bon, u4_atom_cell, &p_bon, &q_bon) ) {
        return u4_and
          (_iris_nest_dext_slip
              (p, sut, bar, axe, gil, bon, nef, ful, u4_noun_2),
           _iris_nest_dext_slip
              (p, sut, bar, axe, gil, bon, nef, ful, u4_noun_3));
      }
      else {
