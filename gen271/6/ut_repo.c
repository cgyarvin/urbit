...
*/
                 ...
                 u2_noun fub)
  {
    if ( u2_no == u2_dust(nog) ) {
      return (u2_nul == nog ? u2_rx(wir_r, fub) : u2_bl_bail(wir_r));
    } else {
             ...
             u2_noun q_sut)
  {
    ...
    u2_noun nog = _repo_hold_nog(wir_r, van, gar);
    u2_noun fub = _repo_hold_fub(wir_r, nog, c3__void);
        ...
        if ( u2_no == u2_as_cell(u2_t(sut), &p_sut, &q_sut)) {
          ...
          if ( u2_yes == j2_mcc(Pit, in, has)(wir_r, fan, sut) ) {
            ...
            u2_noun nav = u2_bn_cook
              (wir_r, van, 
                      "fan", 
                      j2_mcc(Pit, in, put)(wir_r, fan, u2_rx(wir_r, sut)));
            u2_noun mez = _repo_hold(wir_r, nav, p_sut, q_sut);
